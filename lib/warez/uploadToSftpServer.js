...
function uploadToSftpServer(config, ctx, next) {
    ...
    next(null, function(message, content, cb) {
        ...
        var uploadPath = config.folder ? path.join(config.folder, message.qsftp.path) : message.qsftp.path
        ...
        connection.on('ready', function() {
            ...
            connection.sftp(function(err, sftp) {
                ...
                writeStream.on('close', function() {
                    debug(format('Streamed %d bytes to server %s', Buffer.byteLength(content, 'utf8'), url))
                    sftp.end()
                    connection.end()
                    done()
                })
